...
/// ``ComputeRasteriserPointCloud``. Owns the pipeline build (with live
/// reload), auto-allocates the cloud's `displacementBuffer`, flips
/// `ComputeRasteriserConfiguration.applyDisplacement = true` on first encode,
/// and binds the cloud's batches + xyz + levels + displacement buffers each
/// frame. The sketch only writes the kernel + any extra uniforms.
///
/// **Sketch contract.** The user-supplied `.metal` file is concatenated with
/// the package's displacement preamble before compile. The preamble exposes:
///
///   * `RasterBatch` — mirrors the rasteriser's slot metadata.
...
public final class DisplacementPass {
    ...
    //
    // These mirror `SCR_DISP_BUF_*` in the metal preamble. Slots 0–6 are
    // bound by the pass automatically; slots 7–14 are reserved for user
    // uniforms. Beyond 14 is unallocated — collisions with the rasteriser
    // are unlikely but the user is on their own.
    ...
    /// (or before `rasteriser.draw(...)`) so the cloud's depth/color passes
    /// see this frame's deltas.
    public func encode(
        ...
        // Lazily allocate the displacement buffer + flip the rasteriser flag.
        if target.displacementBuffer == nil {
            target.displacementBuffer = target.makeDisplacementBuffer(
                storage: .private,
                label: "DisplacementPass.Displacements"
            )
        }
    ...
    /// Disable the rasteriser's displacement reads. Useful when tearing down
    /// the pass without destroying the cloud (the pass leaves
    /// `applyDisplacement = true` after first encode).
    public func disable() {
    ...
    // MARK: - Preamble
    //
    // Prepended to the user's metal source before `makeLibrary(source:)`.
    // Defines:
    //   * `RasterBatch` (must match `ComputeRasteriserTypes.h` byte-for-byte
    //     — the rasteriser writes into the same MTLBuffer this kernel reads).
    //   * `ScrDisplacementInfo` (must match `InfoData` above).
    //   * `scr_decodePointAt` — mirrors `decodePoint` in Pipelines/Common.metal.
    //   * `scr_resolveDisplacementThread` — slot/localOffset/state gate.
    //   * `SCR_DISP_BUF_*` slot constants + `SCR_DISPLACEMENT_KERNEL_BUFFERS`
    //     macro.
    private static let preamble: String = """
    // Auto-generated displacement preamble — SatinComputeRasteriser.
